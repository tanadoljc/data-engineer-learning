...
def branching_dag():
    ...
    def decider_task(**kwargs):
        # this is Task Instance
        ti = kwargs['ti']
        weekend_flag = ti.xcom_pull(task_ids='fetch_data', key='return_res')['weekend_flag']
        if weekend_flag:
            return 'no_load_data'
        else:
            return 'load_data'
    ...
    fetch_data >> [transform_api_data, transform_db_data, transform_s3_data] >> decider_task() >> [load_data, no_load_data]
